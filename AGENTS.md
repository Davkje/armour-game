...
- **Hosting:** Vercel
- **Multiplayer sync:** PartyKit (built on Cloudflare Workers/Durable Objects) — chosen because it has "rooms" as a first-class primitive (matches the "host game → shareable link → join" requirement) and can filter which part of state each client sees (matches the private-hand requirement below) — both would otherwise need to be hand-built on a generic BaaS. **Confirmed and implemented** (Phase 2 pre-work) — still actively maintained post-Cloudflare-acquisition; no need to separately evaluate Colyseus.
  ...
  - **Implementation:** `party/index.ts` is the room server (one `GameServer` instance per `gameId`, reusing the same id already minted for Local-mode localStorage persistence — no separate room-id scheme). It holds the single authoritative `BoardState` (persisted via `room.storage`, restored in `onStart`), applies incoming `GameAction`s through the same pure `gameReducer` the client uses in Local mode, and after every change broadcasts a **per-connection-redacted** copy of the state — other players' hand cards keep their id (so hand-count badges still work) but have `label`/`imageFront`/`imageBack`/`item` blanked, giving the real server-side hand privacy this doc's Multiplayer State Model section calls for (not just client-side hiding). A disconnected connection's seat is held for a grace period (`DISCONNECT_GRACE_MS`, currently 60s) rather than freed immediately, so a refresh doesn't evict someone from their own hand; the client persists its claimed `PlayerId` in localStorage (`storage.ts`'s `loadOnlinePlayerId`/`saveOnlinePlayerId`) so a returning tab silently reclaims the same seat instead of re-prompting for a name.
  ...
  - Run the room server locally with `yarn party:dev` (alongside `yarn dev`); deploy it with `yarn party:deploy`. The client's `NEXT_PUBLIC_PARTYKIT_HOST` env var points at the deployed host once one exists — falls back to the local `partykit dev` default (`127.0.0.1:1999`) otherwise.
...
- **Phase 2 — Multiplayer (in progress):** real-time shared board state (PartyKit), public/private state split for hands, and a host/invite-link/join flow. Core plumbing landed (see the Tech Stack section's PartyKit note) — homepage's "Online" button mints a `gameId` and routes to `/game/[gameId]?mode=online`, everyone joins through `OnlineJoinScreen` and picks their own name/seat. Still open: production PartyKit deploy, tuning the reconnect grace period, deciding on optimistic UI vs. wait-for-server for online moves, and a "player disconnected" indicator.
