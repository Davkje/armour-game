...
- **Hosting:** Vercel
- **Multiplayer sync:** `partyserver` (Cloudflare's maintained successor to PartyKit) on Cloudflare Workers/Durable Objects, deployed with `wrangler` — chosen because it has "rooms" as a first-class primitive (matches the "host game → shareable link → join" requirement) and lets the server decide what each client sees (matches the private-hand requirement below), both of which would otherwise need to be hand-built on a generic BaaS. **Why not the `partykit` package/platform:** we started on it, but `partykit deploy` to PartyKit's managed platform fails for every new project — `You have exceeded the limit of 10000 Workers custom domains on zone 'partykit.dev'` (partykit/partykit issue #985, open since 2026-06, no maintainer reply; the CLI just POSTs to PartyKit's own `api.partykit.dev`, even for the "deploy to your own Cloudflare account" path, which also needs a custom domain). The `partykit` package was last published 2025-09 while `partyserver` is actively released, and PartyKit's own 2024 announcement said its servers would move "directly inside a Workers/Pages project" — which is what `partyserver` is. Same mental model, so the port was one file (`party/index.ts`).
  - Cost: free at this project's scale, and it cannot turn into an invoice on the Cloudflare Free plan — Cloudflare's Durable Objects pricing says "if you exceed any one of the free tier limits, further operations of that type will fail with an error". Free limits: 100k requests/day and 13,000 GB-s/day duration (billed as 128 MB per object while any tab holds a WebSocket open ≈ 450 GB-s per room-hour, so ~29 room-hours/day free; WebSocket messages count 20:1). Only SQLite-backed Durable Objects are on the Free plan, which is why `wrangler.jsonc` uses `new_sqlite_classes`. Hibernation (`static options = { hibernate: true }`) would stop idle duration charges but is deliberately not enabled — unnecessary at this scale.
  - **Implementation:** `party/index.ts` is the room server (one `GameServer` instance per `gameId`, reusing the same id already minted for Local-mode localStorage persistence — no separate room-id scheme). It holds the single authoritative `BoardState` (persisted via `this.ctx.storage`, restored in `onStart`), applies incoming `GameAction`s through the same pure `gameReducer` the client uses in Local mode, and after every change broadcasts a **per-connection-redacted** copy of the state — other players' hand cards keep their id (so hand-count badges still work) but have `label`/`imageFront`/`imageBack`/`item` blanked, giving the real server-side hand privacy this doc's Multiplayer State Model section calls for (not just client-side hiding). A dropped connection's seat is held for a grace period (`DISCONNECT_GRACE_MS`, 60s, tracked in `heldSeats`) so a newcomer can't take it while its owner refreshes, and the owner can still reclaim it — but a seat with a *live* connection can't be taken over by a rejoin. See the next bullet for how a player gets back into their seat.
  - **Persistence:** Durable Object KV storage caps each *value* at 128 KiB (total per room is unlimited), and a card costs ~400 bytes of state, so the whole `BoardState` can't be one value past ~320 cards. `party/persistence.ts` stores everything except `cards` in one small `meta` value and the cards in chunks of 100 under `cards:N` keys, rewriting only the chunks that changed (it diffs against what it last wrote). Known remaining scaling limits: the server still broadcasts the *full* state to every player on every action, and `FindCardOverlay` renders every card in a pile at once — neither matters until piles reach hundreds of cards.
  - **Client/server contract:** `src/lib/game/protocol.ts` defines the WebSocket message shapes (`ClientMessage`: `join`/`action`/`cursor`; `ServerMessage`: `joined`/`join-rejected`/`roster`/`presence`/`state`/`cursor`). `gameReducer` had to become fully deterministic for this to be safe to run server-side — `SHUFFLE_ZONE` and `PLACE_TOKEN` no longer call `Math.random()`/`crypto.randomUUID()` internally; the randomness/id is generated at the dispatch call site and carried in the action payload instead. `SHUFFLE_ZONE` carries a single `seed` (`createShuffleSeed()` in `reducer.ts`) that the reducer expands with a small seeded PRNG, so it works for a zone of any size — an earlier version carried a fixed-length list of random numbers and silently corrupted the order of any pile over ~65 cards.
  - **Local/Online view swap:** `GameContext.tsx` holds the shared contexts/hooks (`useGameState`, `useGameDispatch`, `useActivePlayerId`, `useGameMode`) that every board component (`Board`, `Zone`, `Card`, etc.) consumes — they never know or care whether they're in Local or Online mode. `GameProvider.tsx` (Local: `useReducer` + localStorage) and `OnlineGameProvider.tsx` (Online: `partysocket` connection, gated behind `OnlineJoinScreen.tsx` until a seat is claimed) are two interchangeable implementations of that same interface. `GameScreen.tsx` is the one place that branches on `?mode=online`. `PlayerSwitcher.tsx` is the one deliberate UI exception that hides itself in Online mode (via `useGameMode()`) since manual player-switching is a local-hotseat-only affordance.
  - **Seats, rejoin and the roster:** every seat that has been sat in has a secret token (`seatTokens`, persisted under its own storage key, never in `BoardState`, the roster or a broadcast). The server sends it only to that seat's owner in the `joined` message; the browser keeps `{playerId, token}` per room in localStorage (`storage.ts`: `loadOnlineSeat`/`saveOnlineSeat`). On every (re)connect the client sends the pair, and a match takes the seat back **even over a still-open stale connection** — a refresh's new socket can arrive before the server notices the old one died, which used to answer "full" or seat you elsewhere. The displaced connection is closed with code 4000 and the client shows "Use this window" instead of auto-reconnecting (otherwise two tabs would keep kicking each other out). A rejoin whose pair doesn't match is answered `join-rejected: unknown-seat` and never quietly seated (that fall-through is what used to skip the name prompt). On a new device there is no token, so the join screen (`OnlineJoinScreen.tsx`) shows a **seat picker** built from the `roster` message the server sends on connect (`{playerId, name, claimed, online}` per seat, updated live for everyone): "Rejoin as Alice (offline)" for an offline claimed seat, or a name box for a never-claimed one. Picking an offline seat needs no proof (honor system; it frees up for newcomers after the 60s grace anyway) but is refused while someone is connected in it, and it rotates the token so the previous holder's saved pair stops working. The menu's Players section (`PlayersPanel.tsx`) shows names, colors (`playerColors.ts`, shared with the cursors), online/offline and a "Copy invite link" button — the invite link is just the room's own address, so there is no separate "original" link.
  - **Server-side validation** (`party/validate.ts`): the room server never applies a client's action as-is. `validateAction` whitelists the action types a client may send (notably **not** `LOAD_STATE`, which would replace the whole game, or `RENAME_PLAYER`), checks that ids/zones/tokens exist, that numbers are sane (finite, bounded positions, integer seeds/rounds, known condition types) and rebuilds the action from known fields only. The one *rule* it enforces is hand privacy — you can't move, flip, tag, shuffle or deal into another player's hand; everything else stays honor-system, in line with the sandbox philosophy. A refused action is dropped and the sender is sent the real board (`sendState`) so its optimistic local prediction snaps back. Messages over 20 KB are ignored; join names are trimmed to 20 chars. Not done: rate limiting (docs' `rate-limiting-messages` guide).
  - **Presence notices:** the server broadcasts `presence` messages (`joined` / `reconnected` / `disconnected`, never to the player it concerns) and `OnlineGameProvider` shows them as short-lived bubbles (`PresenceNotices.tsx`).
  - **Two deploys, not one — the second is automated:** Vercel only builds the Next.js app. The room server (a Cloudflare Worker) is deployed by `.github/workflows/deploy-room-server.yml`, which runs on a push to `main` that touches `party/`, `src/lib/game/`, `src/data/`, `wrangler.jsonc`, `package.json` or `yarn.lock` (it installs, runs `yarn typecheck:party`, then `wrangler deploy` via `cloudflare/wrangler-action`) and can also be started by hand from GitHub's Actions tab. It needs two repository secrets, `CLOUDFLARE_API_TOKEN` (Cloudflare dashboard → My Profile → API Tokens → *Edit Cloudflare Workers* template, limited to this account) and `CLOUDFLARE_ACCOUNT_ID`. The paths matter because `gameReducer`, `initialState.ts`, `types.ts` and `protocol.ts` also run inside the room server. The two deploys run in parallel, so for a minute after a push the site and server can be on different versions — new messages are ignored by old clients and vice versa. `yarn party:deploy` still works for a manual deploy.
  - **Running / deploying:** `yarn party:dev` (= `wrangler dev`, port pinned to 1999 in `wrangler.jsonc`) alongside `yarn dev`; `yarn typecheck:party` type-checks the room server with its own `party/tsconfig.json` (Workers types conflict with the DOM lib, so `party/` is excluded from the root tsconfig). Deploy with `yarn party:deploy` (= `wrangler deploy`, needs a one-time `wrangler login`) to a free `armour-game-party.<account-subdomain>.workers.dev` URL. Set `NEXT_PUBLIC_PARTYKIT_HOST` in Vercel to that bare host (no `https://`) and redeploy — it's baked in at build time; unset it falls back to `127.0.0.1:1999` (which is why a Vercel deployment without it only works for whoever is running `party:dev` locally). partyserver routes `/parties/<kebab-cased binding>/<room>` and has no default party, so the client passes `party: "game-server"` to match the `GameServer` binding. Local DO storage lives in `.wrangler/state` (gitignored).
...
- **Phase 2 — Multiplayer (in progress):** real-time shared board state (partyserver on Cloudflare), public/private state split for hands, and a host/invite-link/join flow. Core plumbing landed (see the Tech Stack section's multiplayer note) — homepage's "Online" button mints a `gameId` and routes to `/game/[gameId]?mode=online`, everyone joins through `OnlineJoinScreen` and picks their own name/seat. Online moves are optimistic (applied locally, server broadcast overwrites), live cursors and the invite-link popup are in. The room server is deployed (`armour-game-party.davkje.workers.dev`) and verified end to end from a phone on mobile data. Still open: an optional shared-password gate on connect and input validation of incoming actions, and a "player disconnected" indicator.
